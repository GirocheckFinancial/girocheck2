...
public class DateTextFilterForm extends BaseFilterForm {
    ...
    private BaseCheckBoxItem showIDs;
    ...
    public DateTextFilterForm() {
        ...
        reportType = new BaseCheckBoxItem("reportType", "Details Report", false);
        reportType.setTextAlign(Alignment.LEFT);
        
Utils.debug("Before build showIDs");
        showIDs = new BaseCheckBoxItem("showIDs", "Show ID", false);
        showIDs.setTextAlign(Alignment.LEFT);
        showIDs.addChangedHandler(new ChangedHandler() { 
            @Override
            public void onChanged(ChangedEvent event) {
                Utils.debug("ChangedEvent");
                Utils.debug("ChangedEvent -> event.getValue() = " + event.getValue());
                SelectShowIDImagesActionExecuted(event.getValue());
            }
        });
        Utils.debug("After build showIDs");
        ...
        setFields(new FormItem[]{startRangeDate, endRangeDate, transactionTypeSelect, operationSelect, space, ammountTypeSelect, opTypeSelect, ammountText, space, searchText, filterButton, pending, importButton, reportType, showIDs});
    }
...
//            Utils.debug( "Exception" );
    ...
    public void SelectShowIDImagesActionExecuted(Object selected) {
        for (SimpleListener listener : listeners) {
            listener.ActionExecuted(selected);
        }
    }
}
