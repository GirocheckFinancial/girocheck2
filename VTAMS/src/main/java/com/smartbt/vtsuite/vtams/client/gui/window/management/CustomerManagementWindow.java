/*
 ** File: CustomerManagementWindow.java
 **
 ...
 */
public class CustomerManagementWindow extends BaseManagementWindow {
     ...
     * @param recordEntity
     */
    public CustomerManagementWindow(Record recordEntity) {
        super();

//        if (Settings.INSTANCE.hasPrivilege(NomUserPrivileges.ALLOW_MERCHANT_TRANSACTIONS)) {
            TransactionTab transactionTab = new TransactionTab(EntityType.CUSTOMER, recordEntity.getAttributeAsInt("id"));
            addTab(transactionTab);
//        }
//        if (Settings.INSTANCE.hasPrivilege(NomUserPrivileges.ALLOW_MERCHANT_CUSTOMER)) {
//            ClientTab clientTab = new ClientTab(EntityType.CUSTOMER, recordEntity.getAttributeAsInt("id"));
//            addTab(clientTab);
//        }
...
//        if (Settings.INSTANCE.hasPrivilege(NomUserPrivileges.ALLOW_MERCHANT_SETTINGS)) {
            SettingsTab settingsTab = new SettingsTab(EntityType.CUSTOMER, recordEntity);
            addTab(settingsTab);
//        }
//        if (Settings.INSTANCE.hasPrivilege(NomUserPrivileges.ALLOW_WATCHDOG)) {
            WatchdogTab watchdogTab = new WatchdogTab(EntityType.CUSTOMER, recordEntity);
            addTab(watchdogTab);
//        }
    }
}
