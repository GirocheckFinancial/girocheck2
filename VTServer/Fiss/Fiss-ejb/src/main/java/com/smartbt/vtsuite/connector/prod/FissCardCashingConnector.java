...
public class FissCardCashingConnector extends Connector {

    private MtvnCWCBPrpdAdjWSV5Interface port;
    private MtvnCWCBPrpdAdjWSV5 service;

    private static FissCardCashingConnector INSTANCE;

    public static synchronized FissCardCashingConnector get() {
        if (INSTANCE == null) {
            INSTANCE = new FissCardCashingConnector();
        }
        return INSTANCE;
    }

    public FissCardCashingConnector() {
        String url = "";
        try {
            ...
            CustomeLogger.Output(CustomeLogger.OutputStates.Debug, ">[FissCardCashingConnector] URL: " + url, null);
