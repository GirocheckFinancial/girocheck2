 ...
 * @author rrodriguez
 */
public class CardCashingReqBuilder {
    ...
    private static CBPrpdAdjMtvnSvcReq.Svc.MsgData buildMsgData(String space, Map<FissParam, String> map, StringBuilder sb) {
        ...
        reqData.setE130488(map.get(FissParam.AMOUNT));
        reqData.setE130486(map.get(FissParam.REASON_CODE));
        reqData.setE131150(map.get(FissParam.DETAILED_TRANSACTION_DESCRIPTION));
        reqData.setE202272(map.get(FissParam.MERCHANT_NAME));
        reqData.setE202273(map.get(FissParam.MERCHANT_CITY_STATE));
        reqData.setE202274("US");

        data.setCBPrpdAdjReqData(reqData);

        sb.append(buildXML("MSG_DATA/REQUEST_DATA", map, space,
                FissParam.CARD_NUMBER,
                FissParam.AMOUNT,
                FissParam.REASON_CODE,
                FissParam.DETAILED_TRANSACTION_DESCRIPTION,
                FissParam.MERCHANT_NAME,
                FissParam.MERCHANT_CITY_STATE
        ));
