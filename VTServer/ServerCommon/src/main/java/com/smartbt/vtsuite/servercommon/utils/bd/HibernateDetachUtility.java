 ...
 */
package com.smartbt.vtsuite.servercommon.utils.bd;
...
/**
 * This is a single static utility that is used to process any object just prior
 * to sending it over the wire to remote clients (like GWT clients or remote web
 * service clients).
 *
 * Essentially this utility scrubs the object of all Hibernate proxies, cleaning
 * it such that it can be serialized over the wire successfully.
 *
 * @author Greg Hinkle
...
public class HibernateDetachUtility {
    ...
    private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(HibernateDetachUtility.class);
     ...
     * @param checkedObjectMap This maps identityHashCodes to Objects we've
     * already detached. In that way we can quickly determine if we've already
     * done the work for the incoming value and avoid taversing it again. This
     * works well almost all of the time, but it is possible that two different
     * objects can have the same identity hash (conflicts are always possible
     * with a hash). In that case we utilize the checkedObjectCollisionMap (see
     * below).
     * @param checkedObjectCollisionMap checkedObjectMap maps the identityhash
     * to the *first* object with that hash. In most cases there will only be
     * mapping for one hash, but it is possible to encounter the same hash for
     * multiple objects, especially on 32bit or IBM JVMs. It is important to
     * know if an object has already been detached because if it is somehow
     * self-referencing, we have to stop the recursion. This map holds the
     * 2nd..Nth mapping for a single hash and is used to ensure we never try to
     * detach an object already processed.
     * @param depth used to stop infinite recursion, defaults to a depth we
     * don't expectto see, but it is configurable.
        ...
        for (Field field : classFields) {
            ...
            if (fieldValue instanceof HibernateProxy) {
                ...
                if (fieldValue instanceof org.hibernate.collection.spi.PersistentCollection) {
                    ...
                    if (fieldValue != null
                            && (fieldValue.getClass().getName().contains("com.smartbt.vtsuite.servercommon.model") || fieldValue instanceof Collection
