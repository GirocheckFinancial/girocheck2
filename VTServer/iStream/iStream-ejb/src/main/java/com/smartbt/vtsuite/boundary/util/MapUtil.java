...
//                    resp = (Integer)map.get(requestParameterName);
                    resp = Integer.parseInt((String)map.get(requestParameterName));
                }catch(ClassCastException e){
                    throw new Exception("Integer value expected from property: " + requestParameterName.toString()+" error "+e.getMessage());
                }
            }else{
                if(required)
    ...
    public static long getLongValueFromMap(Map map, ParameterName requestParameterName, boolean required)throws Exception{
       long resp = 0;
        if(map.containsKey(requestParameterName)){
            if(map.get(requestParameterName) != null){
                try{
//                    resp = (Integer)map.get(requestParameterName);
                    resp = Long.parseLong((String)map.get(requestParameterName));
                }catch(ClassCastException e){
                    throw new Exception("Long value expected from property: " + requestParameterName.toString()+" error "+e.getMessage());
                }
            }else{
                if(required)
                throw new Exception(requestParameterName.toString() + " can't be null.");
        ...
        if(map.containsKey(requestParameterName)){
            if(map.get(requestParameterName) != null){
                try{
                    ...
                    throw new Exception("Double value expected from property: " + requestParameterName.toString());
                }
            }else{
                if(required)
                throw new Exception(requestParameterName.toString() + " can't be null.");
