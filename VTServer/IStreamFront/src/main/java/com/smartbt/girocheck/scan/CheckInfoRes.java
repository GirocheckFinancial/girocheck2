...
@XmlType(name = "CheckInfoRes", propOrder = {
    "checkId",
    "resultCode",
    "resultMessage"
})
public class CheckInfoRes implements IBuilder {
    ...
    @Override
    public CheckInfoRes build(Map map) throws Exception {
        checkId = MapUtil.getStringValueFromMap( map, ParameterName.CHECK_ID, false);
        resultCode = map.get(ParameterName.RESULT_CODE).toString();
        resultMessage = map.get(ParameterName.RESULT_MESSAGE).toString();
        return this;
    }

    public CheckInfoRes mock(String user, String pass) throws Exception {
